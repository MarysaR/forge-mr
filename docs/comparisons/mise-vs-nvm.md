...
- le rôle `development`, désormais implémenté, s'appuie sur mise pour déclarer les versions d'outils dont il a besoin (Node.js, pnpm), plutôt que sur des gestionnaires spécifiques à chaque langage ; le rôle `devops`, désormais implémenté, n'en a pas eu besoin (ses outils ne sont pas des runtimes de langage) ;
